  ...
  <>
    ...
    <section style={{
      ...
      <div className="container">
        ...
        <h1 style={{ fontSize: '2.25rem', fontWeight: 'bold', fontFamily: 'PT Sans, sans-serif', color: '#feff00', margin: '0.5rem 0 0.5rem 0', display: 'inline-block' }}> Heterologous Booster Activity</h1>
        ...
        <p style={{ fontSize: '1rem', marginTop: '0.5rem', marginBottom: 0 }}> 
          Browse a database of curated heterologous COVID-19 booster activity against SARS-CoV-2 variants, along with homologous booster comparisons.<br />
        </p>
        <div style={{ marginTop: '1rem' }}>
          <a href="/data/heterologous_booster.tsv" download style={{ backgroundColor: 'white', color: '#77317d', padding: '0.75rem 1.25rem', borderRadius: '0.5rem', textDecoration: 'none', fontWeight: 'bold', border: '2px solid #77317d' }}>
...
const Footer = () => (
  ...
  useEffect(() => {
    fetch('/data/heterologous_booster.tsv')
      .then(res => res.text())
      .then(tsvText => {
        ...
        setColumnDefs([
          { headerComponent: CustomHeader, 
            ...
            }, field: 'hb_report_number', width: 125 },
          {
            headerComponent: CustomHeader, 
            headerComponentParams: {
          ...
          { headerComponent: CustomHeader, 
            ...
            params.value
              ? params.value
                  ...
                  .join(' ')
              : ''},
          { headerName: 'Primary Vaccine', field: 'vac_prime' },
          { headerName: 'Booster Vaccine', field: 'vac_boost1' },
          { headerName: 'Booster Class', field: 'boost1_class' },
  ...
  const uniqueValues = {
    vacPrime: uniqueValuesFor('vac_prime'),
    vacBoost: uniqueValuesFor('vac_boost1'),
    lineage: uniqueValuesFor('viral_lineage'),
    assayCat: uniqueValuesFor('assay_cat'),
    dataSourceType: uniqueValuesFor('data_source_type')
  };

  const boosterOptions = uniqueValuesFor('vac_boost1');
  const lineageOptions = uniqueValuesFor('viral_lineage');
  const assayCatOptions = uniqueValuesFor('assay_cat');
  const dataSourceTypeOptions = uniqueValuesFor('data_source_type');
...
useEffect(() => {
  const filtered = rowData.filter(row => {
    const vacPrimeMatch =
      filters.vacPrime.length === 0 ||
      filters.vacPrime.includes(row.vac_prime);

    const vacBoostMatch =
      filters.vacBoost.length === 0 ||
      filters.vacBoost.includes(row.vac_boost1);
      ...
      <aside style={{ width: '300px', padding: '1rem 1rem 1rem 5px', backgroundColor: '#f8f9fa', borderRight: '1px solid #ddd', textAlign: 'left', flexShrink: 0 }}>
        ...
        <div>
          <div onClick={() => toggleExpand('vacPrime')} style={{ cursor: 'pointer', color: '#006666', fontWeight: 'bold' }}>
            Vaccine (Prime) <span style={{ float: 'right' }}>{expanded.vacPrime ? '\u25BC' : '\u25B6'}</span>
          </div>
          {expanded.vacPrime && (
            <div style={{ marginLeft: '1rem' }}>
              {(uniqueValues.vacPrime || []).map(val => (
                <label key={val} style={{ display: 'block' }}>
                  <input
                    type="checkbox"
                    checked={filters.vacPrime.includes(val)}
                    onChange={() => handleFilterChange('vacPrime', val)}
                  />{' '}
                  {val}
                </label>
              ))}
